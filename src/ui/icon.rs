...
//!
//! Every shipped icon shares a 16×16 viewBox (matches VSCode codicons). Path data is
...
/// Static registry. Hand-authored 16×16 paths — keep entries alphabetical to ease review.
const PATHS: &[(&str, &str)] = &[
    // Outlined book — replacement for 📚 in the Notes Explorer activity icon.
    (
        "book",
        "M3 2v12c0-.6.4-1 1-1h9V2H4c-.6 0-1 .4-1 1zM4 14a1 1 0 0 0 1 1h8v-1H5z",
    ),
    // Filled circle — tab "dirty" marker (replaces ●).
    ...
    // Hollow rectangle with a right-third splitter — companion-area toggle (was ▸).
    (
        "sidebar-right",
        "M2 3h12v10H2V3zm1 1v8h6V4H3zm7 0v8h3V4h-3z",
    ),
    // Hollow square outline — sidebar "expanded" indicator (replaces ◧).
    (
        "square",
        "M3 3h10v10H3V3zm1 1v8h8V4H4z",
    ),
    // Hollow rectangle with a bottom-third splitter — panel toggle (was ▾).
    (
        "panel",
        "M2 3h12v10H2V3zm1 1v6h10V4H3zm0 7v2h10v-2H3z",
    ),
    // Thin diagonal cross — tab close (replaces ×).
    (
        "x",
