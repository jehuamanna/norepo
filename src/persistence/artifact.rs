...
//! `<repo>/.operon/artifacts/<root-slug>/.../<self-slug>/index.md`. Everything
//! else (markdown, skills, workflows, images) passes through to an inner
//! `Persistence`, unchanged.
//!
//! Per-call overhead: one `find_project_for_note` + one `list_for_project`
//! query (plus one `list()` on the project repo to resolve `repo_path`).
//! Acceptable for save/load latency at expected vault sizes; if a future
//! profile shows this is hot, add a project-scoped notes cache here.
//!
//! Failure semantics: if any lookup fails (note not found, project has no
//! `repo_path`, target isn't an artifact), the wrapper delegates to the
...
impl ArtifactPersistence {
    pub fn new(
        ...
        Self { inner, note_repo, project_repo }
    }
    ...
    /// ids, non-artifact notes, projects without a `repo_path`, missing
    /// slugs).
...
impl Persistence for ArtifactPersistence {
    ...
    fn list<'a>(
        ...
        // per-project `.operon/artifacts/` and the abstract `Persistence::list`
        // doesn't know which project to scan. Callers (search, hydration)
        // query the SQLite note tree directly for artifact discovery.
        self.inner.list()
    }
