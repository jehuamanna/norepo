...
- emit fewer than 3 Features. The downstream pipeline (Stories →
  Tasks → Implementation → Tests → Summary) fans out per Feature, so
  the count drives total breadth.
...
Features under a different Epic. The cascade engine reads this and
sequences Feature-level decomposition (Stories) in topo order.

**How to spot cross-Feature deps within an Epic:**
- **Shared data within the Epic**: Feature X writes a column /
  table / cache key that Feature Y reads.
- **UI flow ordering**: Feature X is a prerequisite step in the
  user journey that Feature Y begins from.
- **Shared modules**: Feature X exposes a util / hook / endpoint
  that Feature Y consumes.

If you can't articulate the dep in one of those terms, leave
`None (parallel-safe)`.

## Calibration
Multi-Feature mode (3–5). Cover the breadth of the Epic; do not
collapse distinct capabilities into a single Feature just to keep
the count small. If the Epic implies more than 5 Features, pick the
5 with the highest leverage and list the rest under `## Out of
scope` of the most-related sibling. If a Feature has only 1
acceptance criterion, it's probably a Story — fold it. If a Feature
has >8 criteria, split the criteria but keep it as one Feature.
