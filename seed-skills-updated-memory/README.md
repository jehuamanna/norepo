# Seed skills (updated-memory variant) — fixed-count 1 / 1 / 1 / 2 BA chain

A trimmed sibling of `seed-skills-updated/` tuned for **small,
single-thread SDLC work** (e.g. building one focused feature into the
app — a memory layer, a single integration, one tightly-scoped MVP).
The cascade shape and Architecture / SDE chain are identical; only the
BA decomposition skills are recalibrated so each level emits a **fixed
count** instead of a flexible 1–3 range:

| Stage   | This variant      | Default (`seed-skills-updated/`) |
|---------|-------------------|----------------------------------|
| Epic    | **1**             | 1–3                              |
| Feature | **1** per Epic    | 1–2                              |
| Story   | **1** per Feature | 1–3                              |
| Task    | **2** per Story   | 1–3                              |

End-to-end: every cascade produces **1 Epic → 1 Feature → 1 Story →
2 Tasks** plus the architecture + per-task plan / implementation /
tests / results subtree. Anything that would naturally fan out wider
gets folded back via `## Out of scope` notes on the absorbing
sibling. Use this variant when you want a deterministic, narrow
breadth and a fast end-to-end run; switch back to
`seed-skills-updated/` for broader features that need genuine
fan-out.

The original chain documentation follows — every behavior other than
the four count knobs above is identical to `seed-skills-updated/`.
