...
**Fixed-count mode (memory variant): emit exactly 1 Feature per
Epic.** Treat the Epic as a single cohesive capability. Even if the
Epic seems to imply two distinct capabilities, fold them into one
Feature and list the deferred capability under `## Out of scope`.
Do NOT emit 2 Features — this variant locks the BA chain to a
1 / 1 / 1 / 2 shape (1 Epic → 1 Feature → 1 Story → 2 Tasks). If a
Feature has only 1 acceptance criterion, that's still fine here —
the Story stage carries the behavior detail. If the Feature would
have >8 criteria, keep them all on one Feature; the Story stage
will narrow scope.
