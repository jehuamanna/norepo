...
not list Stories under other Features. In this slim test pipeline
only one Story is produced per Feature, so `## Depends on` will
always be `None (parallel-safe)`.
...
deferred scope goes under `## Edge cases`.
