//! Hand-rolled SSE parser (mirrors operon-plugins-anthropic/src/sse.rs).
//!
//! OpenAI's streaming API emits frames of the form:
//!   data: {...}\n\n
...
mod tests {
    ...
    async fn parses_done_marker() {
        let r = collect(vec![b"data: [DONE]\n\n"]).await;
        assert_eq!(r.len(), 1);
        assert_eq!(r[0].as_ref().unwrap().data, "[DONE]");
    }

    #[tokio::test]
    async fn handles_split_chunks() {
        let r = collect(vec![b"data: ", b"{\"a\":1}", b"\n\n"]).await;
        assert_eq!(r.len(), 1);
        assert_eq!(r[0].as_ref().unwrap().data, "{\"a\":1}");
    }
}
